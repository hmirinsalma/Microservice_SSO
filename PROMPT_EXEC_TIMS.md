# 🚀 PROMPT D'EXÉCUTION - INTÉGRATION SSO TIMS
...
J'ai 3 applications clientes existantes à intégrer. Tu vas m'aider à intégrer la deuxième : **TIMS (Time & Incident Management System)**.

---

## INFORMATIONS DE L'APPLICATION TIMS

- **Framework Frontend** : React 19.2.7
- **Port Frontend** : http://localhost:5173
- **Backend** : ASP.NET Core
- **Port Backend** : http://localhost:5115
- **Dossier Projet Frontend** : `c:\Users\XPS\Desktop\tims-frontend`
- **Dossier Projet Backend** : `c:\Users\XPS\Desktop\tims-backend`
...
## 🎯 TON TRAVAIL : INTÉGRER LE SSO DANS TIMS AVEC CUSTOM SCOPES

### ÉTAPE 1 : INSTALLATION DES PACKAGES NPM (Frontend)

Dans le dossier frontend, exécute :
```bash
cd c:\Users\XPS\Desktop\tims-frontend
npm install oidc-client-ts react-router-dom
```

---

### ÉTAPE 2 : CRÉER LA CONFIGURATION OIDC AVEC CUSTOM SCOPES
...
### ÉTAPE 3 : CRÉER LE SERVICE D'AUTHENTIFICATION AVEC MÉTHODES CUSTOM TIMS
...
### ÉTAPE 8 : CRÉER L'AXIOS INTERCEPTOR AVEC CUSTOM HEADERS
    ...
    <BrowserRouter>
      <Routes>
        ...
        <Route 
          path="/admin" 
          element={
            <ProtectedRoute requiredRole="ChefServiceTIMS">
              <AdminPage />
            </ProtectedRoute>
          } 
        />
...
```bash
cd c:\Users\XPS\Desktop\tims-backend
...
#### 10.3 Créer le Middleware TIMS Context

Crée le fichier `Middlewares/TimsContextMiddleware.cs` :
...
app.UseTimsContext(); // ⭐ Middleware custom TIMS
...
#### 10.5 Exemple de Controller avec Custom Claims

Crée ou modifie un controller, par exemple `Controllers/IncidentsController.cs` :
...
public class IncidentsController : ControllerBase
{
    ...
    {
        // ⭐ Accéder aux custom claims TIMS
    ...
    public IActionResult CreateIncident([FromBody] IncidentDto dto)
    {
        var timsUserId = HttpContext.Items["TimsUserId"]?.ToString();
        
        // Logique métier avec timsUserId
...
2. Backend TIMS : `cd c:\Users\XPS\Desktop\tims-backend && dotnet run`
3. Frontend TIMS : `cd c:\Users\XPS\Desktop\tims-frontend && npm run dev`

### Test 2 : Login avec custom claims
1. Aller sur http://localhost:5173
2. Cliquer "Se connecter avec ONEE SSO"
3. Login : `admin@onee.ma` / `Admin@123`
4. **Vérifier dans la console** : custom claims `tims_user_id`, `tims_service_id`, `tims_team_id`
5. Vérifier redirection vers `/dashboard`

### Test 3 : API Call avec custom headers
1. Dans Dashboard, faire un appel API GET `/api/incidents`
2. Ouvrir DevTools → Network → Vérifier les headers de la requête
3. Vérifier que `X-TIMS-User-Id`, `X-TIMS-Service-Id`, `X-TIMS-Team-Id` sont présents
...
✅ Login → Redirection SSO → Callback avec custom claims TIMS  
✅ Console affiche `tims_user_id`, `tims_service_id`, `tims_team_id`  
✅ Toutes les requêtes API incluent token + headers `X-TIMS-*`  
...
2. ✅ Crée tous les fichiers React avec custom scopes TIMS
3. ✅ Installe le package NuGet dans le backend
4. ✅ Crée le middleware `TimsContextMiddleware.cs`
5. ✅ Modifie `appsettings.json` et `Program.cs` du backend
6. ✅ Crée un exemple de controller avec custom claims
7. ✅ Valide que tous les fichiers sont créés
