...
- **ClientId** : `tims-app`
- **Scopes** : `openid profile email roles offline_access tims`
...
## 🔐 ÉTAPE 3 : SERVICE D'AUTHENTIFICATION TIMS
...
const API_BASE_URL = 'http://localhost:5115/api'; // Backend TIMS
...
### 8.4 Middleware pour Custom Claims TIMS

Créer `Middlewares/TimsContextMiddleware.cs` :
...
### 8.5 Protéger les Controllers avec Custom Claims
...
public class IncidentsController : ControllerBase
{
    ...
    public IActionResult GetIncidents()
    {
        ...
        var userId = User.FindFirst("tims_user_id")?.Value;
    ...
    [HttpPost]
    [Authorize(Roles = "ChefServiceTIMS")] // ✅ Rôle spécifique requis
...
2. Lancer le backend TIMS : `dotnet run` (port 5115)
3. Lancer le frontend TIMS : `npm run dev` (port 5173)
...
8. **Vérifier dans la console** : custom claims `tims_user_id`, `tims_service_id`, `tims_team_id`

### Test 2 : Custom Claims dans API
1. Faire un appel API GET `/api/incidents`
2. Vérifier que les headers `X-TIMS-User-Id`, `X-TIMS-Service-Id`, `X-TIMS-Team-Id` sont présents
...
const Dashboard = () => {
  ...
  const loadUserData = async () => {
    ...
    console.log('🎯 TIMS Context:', context);
  };
    ...
    <div>
      <header>
        ...
        <div>
          ...
          <span>🆔 TIMS User ID: {timsContext?.userId}</span>
          <span>🏢 Service: {timsContext?.serviceId}</span>
          <span>👥 Team: {timsContext?.teamId}</span>
      ...
      <main>
        ...
        <ul>
          {incidents.map(inc => (
            <li key={inc.id}>{inc.title} - {inc.status}</li>
          ))}
        </ul>
      </main>
    </div>
...
- [ ] `authConfig.js` avec scopes TIMS créé
- [ ] `authService.js` avec méthodes custom TIMS créé
- [ ] Pages Login et Callback créées
- [ ] `silent-renew.html` créé
- [ ] `ProtectedRoute` créé
- [ ] `axiosConfig.js` avec headers custom créé
- [ ] Routes configurées dans `App.jsx`
- [ ] Backend JWT configuré
- [ ] Middleware `TimsContext` créé et configuré
- [ ] CORS configuré
- [ ] Controllers protégés avec `[Authorize]`
- [ ] Test login réussi ✅
- [ ] Test custom claims TIMS reçus ✅
...
✅ Login sur le SSO → Callback avec custom claims TIMS
✅ Toutes les requêtes API incluent token + headers `X-TIMS-*`
✅ Le backend extrait les claims `tims_user_id`, `tims_service_id`, `tims_team_id`
...
En cas de problème :
1. Vérifier les ports (5205 SSO, 5115 Backend TIMS, 5173 Frontend TIMS)
2. Vérifier la console pour les custom claims
3. Vérifier les logs backend pour les headers `X-TIMS-*`
4. Vérifier que le ClientId `tims-app` existe dans la base SSO

🚀 **Votre application TIMS est maintenant intégrée avec ONEE.SSO + Custom Claims !**
